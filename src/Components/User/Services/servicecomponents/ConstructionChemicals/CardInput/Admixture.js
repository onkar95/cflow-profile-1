...
function Admixture({ formData, setNewRequest, setOpenSaved, quantity, setQuantity, newRequest, setModalOpen, setData, chemicalDropDown }) {
    const [userId, setUserId] = useState(JSON.parse(localStorage.getItem('profile'))?.data?.id)
    const history = useHistory()
    const [selectedbrand, setSelectedBrand] = useState(null)
    const [selectedtype, setSelectedType] = useState(null)

    let types = chemicalDropDown?.concrete_admixture_types;
    let brands = chemicalDropDown?.concrete_admixture_brands;
