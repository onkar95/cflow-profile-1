...
function Grout({ formData, setNewRequest, setOpenSaved, quantity, setQuantity, newRequest, setModalOpen, setData, chemicalDropDown }) {
    const [userId, setUserId] = useState(JSON.parse(localStorage.getItem('profile'))?.data?.id)
    const history = useHistory()
    const types = ["Expanding grout admixture", "Cement grout", "Epoxy grout", "Polyester resin grout", "Demoulding agents", "Surface retarders", "Rust removers", "Concrete cleaners"]
    const brands = ["Thremax", "Carbolink", "Fosroc", "Sika", "Bostik", "ARDEX"]
