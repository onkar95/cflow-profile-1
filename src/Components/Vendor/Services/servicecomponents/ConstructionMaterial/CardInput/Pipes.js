...
function Pipes({ formData, notify, setModalOpen, newService, setNewService, AddService, handleClickOpen, dropdownData }) {
  ...
  useEffect(() => {
    ...
    brandtemp.map((type) => {
      ...
      let final_dict = { type: type }
      final_arr.push(final_dict)

    })

    setNewService({ ...newService, info: final_arr, type: "Pipes" })
  }, [typecheck])
